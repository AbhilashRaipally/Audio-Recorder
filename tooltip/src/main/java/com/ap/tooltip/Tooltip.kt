...
private const val TooltipTimeout = 1_000L - OutTransitionDuration
 ...
 *
 * @param visibility Whether the tooltip is currently visible to the user
...
fun Tooltip(
    ...
    expandedStates.targetState = visibility.value

    if (expandedStates.currentState || expandedStates.targetState) {
        if (expandedStates.isIdle) {
            LaunchedEffect(timeoutMillis, visibility) {
                delay(timeoutMillis)
